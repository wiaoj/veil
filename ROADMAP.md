...
- [x] JA3 + JA4 TLS fingerprinting — ClientHello peeked off the socket (`TcpStream::peek`, non-consuming) and parsed into a JA3 MD5 (`edge/src/tls/ja3.rs`) and a JA4 fingerprint (`edge/src/tls/ja4.rs`, FoxIO: version/SNI/counts/ALPN + sorted cipher & extension SHA-256, GREASE-stripped, resilient to extension-order randomisation); both exposed on the request context and as `ja3`/`ja4` rule conditions for blocklisting bot/tooling fingerprints. Verified against a **real** rustls ClientHello (`edge/tests/tls_fingerprint.rs`), not just hand-built fixtures. **Limitation:** the fingerprints only exist when the edge terminates TLS itself — behind a TLS-terminating load balancer there is no ClientHello to peek, so `ja3`/`ja4` are `None` and any rule/signal using them silently never matches
- [x] TLS/User-Agent contradiction signal — the header heuristics are trivially forged (a bot sending a full browser header set scores 0), but the TLS fingerprint is a property of the client's TLS stack. A UA claiming a mainstream browser whose ClientHello lacks SNI or ALPN `h2` is scored +30 (`risk::tls_contradicts_user_agent`). Uses only structural JA4 parts — no fingerprint database to maintain
...
- [x] Managed signature rule set (OWASP-CRS-style) — built-in SQLi/XSS/path-traversal `RegexSet` families (`pipeline/signatures.rs`), per-zone `managed_rules` toggle + block/challenge action, scanning URL/query/**all headers** (credential headers skipped) and the **decoded** body (gzip/deflate, bounded against decompression bombs); regex rule conditions (`path_regex`/`query_regex`/`header_regex`/`body_regex`) plus **`body_json`** (regex over one JSON field by dotted path — far fewer false positives than whole-body scanning); opt-in body buffering (256 KiB cap) with an optional `block_oversized_body` to reject over-cap bodies instead of forwarding them un-inspected. Starter set, not a full CRS port
- [ ] Streaming body inspection — scan the body as it flows (overlapping window across chunk boundaries) instead of buffering to a 256 KiB cap, so a large legitimate upload is inspected without either a memory-DoS risk or the current oversized-body bypass. Non-trivial: streaming a `RegexSet` over unbounded input
- [~] Positive request validation (JSON Schema contract enforcement) — negative signatures catch *known* attacks; a schema describes the *valid* shape and rejects everything else. Built inside the negative model: the `body_schema` rule condition matches when a JSON body doesn't conform (`jsonschema` crate, offline only — no remote `$ref`, so a body can't make the edge fetch a URL), so pairing it with `block` enforces a contract and **shadow mode logs the would-be rejects for free** (mandatory rollout discipline, since schema validation fails *closed*).
  - [x] **Phase 1 — edge primitive:** `Condition::BodySchema`, compiled once at config load (invalid schema fails the load); usable today via `veil.json`
  - [x] **Phase 2 — Vaultify integration:** the `body_schema` rule condition carries a *reference* (`subject`/`version`) into the **Vaultify schema registry** (sibling repo — it validates + SemVer-versions + compatibility-checks on upload, so Veil needs no JSON-Schema validator of its own). `ISchemaRegistry`/`VaultifySchemaRegistry`: `POST /v1/schemas` uploads (dashboard → Vaultify), and ConfigSync resolves every referenced schema to its concrete form and embeds it in the snapshot at push time. A reference that can't be resolved is dropped **fail-open** (skip validation, never break the snapshot). Off when `Vaultify:BaseUrl` is unset
  - [x] **Phase 3 — dashboard:** `body_schema` in the rule builder with subject/version inputs
  - [ ] Dashboard schema *upload* form (paste JSON → `POST /v1/schemas`) — the endpoint exists; the UI is still a raw reference today
...
- [x] Branded gateway/origin error pages — upstream failures render the shared branded page (HTML for browsers, `problem+json` otherwise, EN/TR), classified into distinct reasons (`web_server_down` / `origin_unreachable` / `origin_ssl_handshake` / `origin_ssl_invalid` / `origin_timeout` / `origin_bad_response` / `bad_gateway`) over standard 502/503/504 wire codes (not Cloudflare-style 52x) with an `x-veil-error` reference header. Adds upstream connect + response timeouts (`VEIL_UPSTREAM_CONNECT_TIMEOUT_SECS` / `VEIL_UPSTREAM_TIMEOUT_SECS`); empty-config nodes serve a branded `503`
- [~] Integration test suite (Testcontainers) — `tests/Veil.IntegrationTests` (xUnit) covers the analytics data clients against real PostgreSQL (`NpgsqlIncidentArchive`, `DailySummaryStore`) and ClickHouse (`ClickHouseWriter`/`Reader`, incl. the `asn` round-trip). Redis-backed paths + full HTTP-level API tests still to come
...
- [x] Edge response caching (opt-in per zone) — conservative subset of RFC 7234 (`edge/src/response_cache.rs`): only `GET` without `Authorization`/`Cookie`, only `200` with no `Set-Cookie`/`Vary` and **explicit** `Cache-Control: s-maxage`/`max-age`, only bodies with a known `Content-Length` within the zone cap. Bounded in-memory store, `X-Veil-Cache: HIT/MISS`. Toggled per zone from the dashboard (`PUT /v1/zones/{id}/cache` → `cache_enabled` column → pushed to the edge as a `cache` object)
...
- [x] Rolling traffic-window features **in memory** off the live ingest stream (req rate, verdict mix, top IPs/paths/**ASNs**, distinct IPs, **challenge pass rate**) per zone — no ClickHouse round-trip, so detection is instant.
- [x] **ML.NET** spike detection (`MlAnomalyDetector`, IID spike over the per-zone rate series) + deterministic attack signals (block ratio / single-source share / **single-ASN share** for distributed floods / **low challenge-pass rate** for bots failing the challenge) — always-on, in-process, no LLM
...
- [x] Challenge-pass signal — uses the existing `challenge`/`challenge_pass` verdicts: a low solve rate under real volume flags `automated_clients` and **escalates** the single-source/ASN suggestion from challenge to block (the challenge screen isn't stopping them)
